...
function Administrator() {
  ...
  const sendPodcastToDatabase = (e) => {
    ...
    const result = axios
      .post("http://localhost:5000/app/savePodcastInDatabase", onePodcast)
  ...
  const getDataFromBase = () => {
    ...
    if (selectedTag == "user") {
      const result = axios
        .get("http://localhost:5000/app/getAllUsersFromDatabase")
    ...
    if (selectedTag == "podcast") {
      ...
      const result = axios
        .get("http://localhost:5000/app/getAllPodcastFromDatabase")
  ...
  const deleteUser = (e) =>{
    ...
    const result = axios
        .post("http://localhost:5000/app/deleteUsers",{idOfUser})
        .then((res) => setAllUsers(res.data));
  }
  ...
  const deletePodcast = (e)=>{
    ...
    const result = axios
        .post("http://localhost:5000/app/deletePodcast",{idOfPodcast})
        .then((res) => setAllPodcast(res.data));
  }
  ...
  const saveAdministratorChanges = (e) =>{
    ...
    const result = axios.post("http://localhost:5000/app/changeAdministratorValue",data)
                        .then((res) => setAllUsers(res.data));
  }
