...
function Administrator() {
  ...
  useEffect(() => {
    ...
    if (isCookie) {
      ...
      axios.post("/app/checkCookie", cookieSend).then((res) => checkRes(res));
  ...
  const sendPodcastToDatabase = (e) => {
    ...
    const checkStatus = (res) => {
      if (res.status === 200) {
        document.getElementById("nameOfPodcast").value ="";
        document.getElementById("chanelId").value = "";
  ...
  const getDataFromBase = () => {
    ...
    if (selectedTag === "user") {
    axios.get("/app/getAllUsersFromDatabase")
          .then((res) => setAllUsers(res.data));
      setAllPodcast(undefined);
    }

    if (selectedTag === "podcast") {
       axios.get("/app/getAllPodcastFromDatabase")
  ...
  const deleteUser = (e) => {
    const idOfUser = e.target.dataset.id;
     axios.post("/app/deleteUsers", { idOfUser })
          .then((res) => setAllUsers(res.data));
  };

  const deletePodcast = (e) => {
    const idOfPodcast = e.target.dataset.id;
     axios.post("/app/deletePodcast", { idOfPodcast })
          .then((res) => setAllPodcast(res.data));
  };
  ...
  const saveAdministratorChanges = (e) => {
    ...
    const data = {
     ...
     axios.post("/app/changeAdministratorValue", data)
          .then((res) => setAllUsers(res.data));
  };
    ...
    <main id="mainDivAdministrator">
      ...
      <div id="divForInputPodcast">
        ...
        <input
          ...
          id="btnSubmit"
          value="SAVE"
          onClick={(e) => sendPodcastToDatabase(e)}
        />
      </div>
        ...
        <div id="divForData">
            ...
            <div className="oneUser" key={elem._id}>
              ...
              <div>
                ...
                <select
                  name="administrator"
                  id="administrator"
                  defaultValue={elem.administrator == 1 ? "1" : "0"}
                >
              ...
              <div className="buttonsOneUser">
                <button onClick={(e) => deleteUser(e)} data-id={elem._id}>
                  delete user
                </button>
                <button
                  onClick={(e) => saveAdministratorChanges(e)}
                  data-id={elem._id}
                >
                  save the administrator change
                </button>
              </div>
            </div>
          ))}
          {allPodcast?.map((elem) => (
            <div className="onePodcastAdministratorPage" key={elem._id}>
              ...
              <button
                className="deletePodcastBtn"
                data-id={elem._id}
                onClick={(e) => deletePodcast(e)}
              >
                Delete podcast
              </button>
            </div>
